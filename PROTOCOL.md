...
**Protocol version 5.** This document is the authority for the version number; bump it here when a shape changes incompatibly.

Version 5 added Client-opened Usage Reports (`report-link-request` §5.5, `report-link` §6.11, and the temporary browser exchange in §6.12). Additive both ways: an older Client has no Report menu; a version 5 Client against an older server times out after ten seconds and explains that the server does not support Usage Reports. All enforcement and reporting continue unchanged.
...
### 5.5 `report-link-request` · implemented

Sent when someone chooses one of the four Usage Report periods from the Client tray. Live-only: the menu is disabled while the Client is offline.

```json
{ "type": "report-link-request", "requestId": "7f3c…", "days": 30 }
```

| Field | Type | Notes |
|---|---|---|
| `requestId` | string | Random correlation id generated by the Client and echoed in `report-link` (§6.11). Not a credential. |
| `days` | int | Exactly `7`, `30`, `90`, or `120`. The issued capability is bound to this period. |

The server takes the Client identity from the authenticated socket, never from this payload.
...
### 6.11 `report-link` · implemented

The correlated answer to `report-link-request` (§5.5).

```json
{
  "type": "report-link",
  "requestId": "7f3c…",
  "path": "/clients/3/report?days=30#token=64-hex-character-bearer-token"
}
```

`path` is server-relative and resolved against the paired base URL (§2). It is null when issuance is refused. The token is in the URL fragment, which browsers do not send in HTTP request lines or `Referer` headers.

The Report Link is reusable for a fixed 30 minutes and grants only the named Client and period. Up to 16 may coexist per Client; issuing another removes the oldest. Re-Pairing, revoking, or deleting the Client invalidates all of its links. Pausing enforcement does not.

### 6.12 Usage Report browser exchange · implemented

An unauthenticated `GET /clients/:id/report?days=N` returns only a bootstrap page. That page removes `#token` from browser history and submits it in the body of `POST /clients/:id/report/access?days=N`. A valid exchange sets a fixed-expiry, HttpOnly, same-site cookie scoped to that Client's report path, then redirects to the clean GET URL.

The clean report accepts either an Admin session or the scoped Report Link cookie. Invalid, expired, altered, or invalidated links receive the same localized `403` response. Report and exchange responses use `Cache-Control: no-store` and `Referrer-Policy: no-referrer`. The bearer token is never logged and is stored server-side only as a SHA-256 digest in process memory; a server restart therefore invalidates outstanding links.
